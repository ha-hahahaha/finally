...
$(document).ready(function() {
	...
	$.ajax({
		url: "SelectSortsServlet",
		type: "POST",
		dataType: "json",
		success: function(data) {
			for(var i = 0; i < data.length; i++) {
				$(".topic").append("<button id='sort'>"+data[i].sortName+"</button>");
			...
			success: function(data) {
				...
				for(var i = 0; i < data.length; i++) {
					if(title == data[i].title) {
						$("#news").append("<div class='title'>" + title + "</div><div class='remark'>" + data[i].writeTime + " " + data[i].author + "</div><div class='text'>" + data[i].content + "</div><div class='pre'><img src='img/shangyitiao.png'></div><div class='next'><img src='img/xiayitiao.png'></div><a href='news.html'><div class='close'>关闭窗口</div></a>");
						num = i;
					}
				}
			},
	...
	$('body').on('click', '.pre img', function() {
		...
		$.ajax({
			...
			success: function(data) {
				...
				if(num != 0) {
					$("#news").empty();
					$("#news").append("<div class='title'>" + data[num - 1].title + "</div><div class='remark'>" + data[num - 1].writeTime + " " + data[num - 1].author + "</div><div class='text'>" + data[num - 1].content + "</div><div class='pre'><img src='img/shangyitiao.png'></div><div class='next'><img src='img/xiayitiao.png'></div><a href='news.html'><div class='close'>关闭窗口</div></a>");
					num = num - 1;
				}
			},
			error: function() {
