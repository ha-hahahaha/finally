$(document).ready(function() {
	...
	//显示所有子类
	for(var i = 0; i < num; i++) {
		id = $("ul").eq(i).children('li').eq(1).html();
		$.ajax({
			...
			success: function(data) {
				...
				for(var j = 0; j < data.length; j++) {
					$("ul").eq(i).append("<li class='item'><img src='img/tuya_huabanfuben.png'><a href='" + data[j].toLink + "'>" + data[j].navName + "</a><div id='changeson'>更改</div><div id='deleteson'>删除</div></li>")
				}
			},
			error: function() {
				var error = "查询数据失败！";
